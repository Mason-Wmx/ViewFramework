#include "vtkExtSimOpenGLPolyDataMapper.h"
...
#endif
#include "vtkExtSimValuePassHelper.h"
...
//-----------------------------------------------------------------------------
vtkStandardNewMacro(vtkExtSimOpenGLPolyDataMapper)

//-----------------------------------------------------------------------------
vtkExtSimOpenGLPolyDataMapper::vtkExtSimOpenGLPolyDataMapper()
  : UsingScalarColoring(false)
{
  ...
  this->VBO = vtkExtSimOpenGLVertexBufferObject::New();
  ...
  this->TimerQuery = 0;
  this->ResourceCallback = new vtkOpenGLResourceFreeCallback<vtkExtSimOpenGLPolyDataMapper>(this,
    &vtkExtSimOpenGLPolyDataMapper::ReleaseGraphicsResources);
#if GL_ES_VERSION_2_0 != 1 && GL_ES_VERSION_3_0 != 1
  this->ValuePassHelper = vtkSmartPointer<vtkExtSimValuePassHelper>::New();
#endif
}
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::ReleaseGraphicsResources(vtkWindow* win)
{
...
void vtkExtSimOpenGLPolyDataMapper::AddShaderReplacement(
    ...
    bool replaceAll)
{
  vtkExtSimOpenGLPolyDataMapper::ReplacementSpec spec;
  spec.ShaderType = shaderType;
  spec.OriginalValue = originalValue;
  spec.ReplaceFirst = replaceFirst;

  vtkExtSimOpenGLPolyDataMapper::ReplacementValue values;
...
void vtkExtSimOpenGLPolyDataMapper::ClearShaderReplacement(
    vtkShader::Type shaderType, // vertex, fragment, etc
    std::string originalValue,
    bool replaceFirst)
{
  ...
  typedef std::map<const vtkExtSimOpenGLPolyDataMapper::ReplacementSpec,
    vtkExtSimOpenGLPolyDataMapper::ReplacementValue>::iterator RIter;
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::BuildShaders(
    ...
    vtkRenderer *ren, vtkActor *actor)
{
  ...
  typedef std::map<const vtkExtSimOpenGLPolyDataMapper::ReplacementSpec,
    vtkExtSimOpenGLPolyDataMapper::ReplacementValue>::const_iterator RIter;
...
//-----------------------------------------------------------------------------
bool vtkExtSimOpenGLPolyDataMapper::HaveWideLines(
  vtkRenderer *ren,
  vtkActor *actor)
{
...
//-----------------------------------------------------------------------------
vtkMTimeType vtkExtSimOpenGLPolyDataMapper::GetRenderPassStageMTime(vtkActor *actor)
{
...
//-----------------------------------------------------------------------------
bool vtkExtSimOpenGLPolyDataMapper::HaveTextures(vtkActor *actor)
{
  return (this->GetNumberOfTextures(actor) > 0);
}

//-----------------------------------------------------------------------------
unsigned int vtkExtSimOpenGLPolyDataMapper::GetNumberOfTextures(vtkActor *actor)
{
  unsigned int res = 0;
...
//-----------------------------------------------------------------------------
std::vector<vtkTexture *> vtkExtSimOpenGLPolyDataMapper::GetTextures(vtkActor *actor)
{
...
//-----------------------------------------------------------------------------
bool vtkExtSimOpenGLPolyDataMapper::HaveTCoords(vtkPolyData *poly)
{
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::GetShaderTemplate(
    std::map<vtkShader::Type, vtkShader *> shaders,
    vtkRenderer *ren, vtkActor *actor)
{
...
//------------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderRenderPass(
...
//------------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderColor(
  std::map<vtkShader::Type, vtkShader *> shaders,
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderLight(
  std::map<vtkShader::Type, vtkShader *> shaders,
  vtkRenderer *, vtkActor *actor)
{
  std::string FSSource = shaders[vtkShader::Fragment]->GetSource();
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderTCoord(
  std::map<vtkShader::Type, vtkShader *> shaders,
  vtkRenderer *, vtkActor *actor)
{
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderPicking(
  std::map<vtkShader::Type, vtkShader *> shaders,
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderClip(
  std::map<vtkShader::Type, vtkShader *> shaders,
  vtkRenderer *, vtkActor *)
{
  std::string VSSource = shaders[vtkShader::Vertex]->GetSource();
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderNormal(
  std::map<vtkShader::Type, vtkShader *> shaders,
  vtkRenderer *, vtkActor *actor)
{
  std::string FSSource = shaders[vtkShader::Fragment]->GetSource();
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderPositionVC(
  std::map<vtkShader::Type, vtkShader *> shaders,
  vtkRenderer *, vtkActor *)
{
  std::string VSSource = shaders[vtkShader::Vertex]->GetSource();
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderPrimID(
  std::map<vtkShader::Type, vtkShader *> shaders,
  vtkRenderer *, vtkActor *)
{
  std::string VSSource = shaders[vtkShader::Vertex]->GetSource();
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderCoincidentOffset(
  std::map<vtkShader::Type, vtkShader *> shaders,
  vtkRenderer *ren, vtkActor *actor)
{
  float factor = 0.0;
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderDepth(
    std::map<vtkShader::Type, vtkShader *> shaders,
    vtkRenderer *, vtkActor *)
{
...
void vtkExtSimOpenGLPolyDataMapper::ReplaceShaderValues(
  std::map<vtkShader::Type, vtkShader *> shaders,
  vtkRenderer *ren, vtkActor *actor)
{
...
bool vtkExtSimOpenGLPolyDataMapper::DrawingSpheres(vtkOpenGLHelper &cellBO, vtkActor *actor)
{
...
bool vtkExtSimOpenGLPolyDataMapper::DrawingTubes(vtkOpenGLHelper &cellBO, vtkActor *actor)
{
...
//-----------------------------------------------------------------------------
bool vtkExtSimOpenGLPolyDataMapper::GetNeedToRebuildShaders(
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::UpdateShaders(
  vtkOpenGLHelper &cellBO, vtkRenderer* ren, vtkActor *actor)
{
  vtkOpenGLRenderWindow *renWin = vtkOpenGLRenderWindow::SafeDownCast(ren->GetRenderWindow());
...
void vtkExtSimOpenGLPolyDataMapper::SetMapperShaderParameters(vtkOpenGLHelper &cellBO,
                                                      vtkRenderer* ren, vtkActor *actor)
{
      ...
      cellBO.ShaderSourceTime > cellBO.AttributeUpdateTime))
  {
    ...
    auto mapper = vtkExtSimOpenGLPolyDataMapper::SafeDownCast(actor->GetMapper());
    ...
    if (mapper)
    {
        ...
        for (int i = 0;i < pointData->GetNumberOfArrays();i++)
        {
            ...
            if (cellBO.Program->IsAttributeUsed(ar->GetName()))
            {
                if (!cellBO.VAO->AddAttributeArray(cellBO.Program, this->VBO,
                    ar->GetName(), this->VBO->AttributesOffset[ar->GetName()],
                    this->VBO->Stride, VTK_FLOAT, this->VBO->AttributesComponents[ar->GetName()], false))
                {
                    vtkErrorMacro(<< "Error setting custom attribute in shader VAO.");
                }
            }
        }
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::SetLightingShaderParameters(
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::SetCameraShaderParameters(vtkOpenGLHelper &cellBO,
                                                    vtkRenderer* ren, vtkActor *actor)
{
  vtkShaderProgram *program = cellBO.Program;
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::SetPropertyShaderParameters(vtkOpenGLHelper &cellBO,
                                                       vtkRenderer*, vtkActor *actor)
{
  vtkShaderProgram *program = cellBO.Program;
...
void vtkExtSimOpenGLPolyDataMapper::GetCoincidentParameters(
  vtkRenderer* ren, vtkActor *actor,
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::RenderPieceStart(vtkRenderer* ren, vtkActor *actor)
{
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::RenderPieceDraw(vtkRenderer* ren, vtkActor *actor)
{
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::RenderPieceFinish(vtkRenderer* ren,
  vtkActor *actor)
{
  vtkHardwareSelector* selector = ren->GetSelector();
  // render points for point picking in a special way
...
//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::RenderPiece(vtkRenderer* ren, vtkActor *actor)
{
...
void vtkExtSimOpenGLPolyDataMapper::RenderEdges(vtkRenderer* ren, vtkActor *actor)
{
  vtkProperty *prop = actor->GetProperty();
...
//-------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::ComputeBounds()
{
  if (!this->GetInput())
  {
...
//-------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::UpdateBufferObjects(vtkRenderer *ren, vtkActor *act)
{
...
//-------------------------------------------------------------------------
bool vtkExtSimOpenGLPolyDataMapper::GetNeedToRebuildBufferObjects(
...
void vtkExtSimOpenGLPolyDataMapper::AppendCellTextures(
  vtkRenderer *ren,
  vtkActor *,
...
void vtkExtSimOpenGLPolyDataMapper::BuildCellTextures(
  vtkRenderer *ren,
  vtkActor *actor,
  vtkCellArray *prims[4],
  int representation)
...
// to their cell id
vtkPolyData *vtkExtSimOpenGLPolyDataMapper::HandleAppleBug(
  vtkPolyData *poly,
...
//-------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::BuildBufferObjects(vtkRenderer *ren, vtkActor *act)
{
  ...
  toString << poly->GetMTime() <<
	...
	'D' << (poly->GetPointData() ? poly->GetPointData()->GetMTime() : 1);
  ...
  if (this->VBOBuildString != toString.str())
  {
    ...
    this->VBO->CreateVBO(poly->GetPoints(),
        ...
        c ? c->GetNumberOfComponents() : 0, poly->GetPointData());
...
//-------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::BuildIBO(
  vtkRenderer *ren,
  vtkActor *act,
  vtkPolyData *poly)
{
...
//-----------------------------------------------------------------------------
bool vtkExtSimOpenGLPolyDataMapper::GetIsOpaque()
{
...
//----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::ShallowCopy(vtkAbstractMapper *mapper)
{
  vtkExtSimOpenGLPolyDataMapper *m = vtkExtSimOpenGLPolyDataMapper::SafeDownCast(mapper);
  if (m != NULL)
  {
...
void vtkExtSimOpenGLPolyDataMapper::SetVBOShiftScaleMethod(int m)
{
  this->VBO->SetCoordShiftAndScaleMethod(
    static_cast<vtkExtSimOpenGLVertexBufferObject::ShiftScaleMethod>(m));
}

//-----------------------------------------------------------------------------
void vtkExtSimOpenGLPolyDataMapper::PrintSelf(ostream& os, vtkIndent indent)
{
