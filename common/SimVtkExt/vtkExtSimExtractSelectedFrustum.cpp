...
#include "stdafx.h"
#include "vtkExtSimExtractSelectedFrustum.h"
...
vtkExtSimExtractSelectedFrustum* vtkExtSimExtractSelectedFrustum::New()
{
	VTK_STANDARD_NEW_BODY(vtkExtSimExtractSelectedFrustum)
}

SIM::vtkExtSimExtractSelectedFrustum::vtkExtSimExtractSelectedFrustum():RespectBoundCells(0), _cellCache(nullptr)
{
}

int vtkExtSimExtractSelectedFrustum::RequestData(
	...
	if (this->FieldType == vtkSelectionNode::CELL)
	{
		...
		*/
		...
		for (cellId = 0; cellId < numCells; cellId++)
		{
			...
			if (_cellCache && _cellCache->find(cellId) != _cellCache->end())
			{
				auto ar = (*_cellCache)[cellId];
				for (int i = 0;i < 6;i++)
				{
					bounds[i] = ar[i];
				}
			...
			isect = this->BoxFrustumIsect(bounds, cell);
...
int vtkExtSimExtractSelectedFrustum::BoxFrustumIsect(double *bounds, vtkCell *cell)
{
	...
	if (!RespectBoundCells)
	{
		auto cellPoints = cell->GetPoints();

		for (int k = 0; k < cellPoints->GetNumberOfPoints(); k++)
		{
			auto pt = cellPoints->GetPoint(k);
			for (int pid = 0; pid < 6; pid++)
			{
				vtkPlane *plane = Frustum->GetPlane(pid);
				double dist;
